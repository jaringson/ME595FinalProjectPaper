...
This along with an efficient evolutionary algorithm seeks to decrease the optimization search space while also approximating the underlying model. % Phil's work did this, so I'm not sure if this is trying to say we want to do this or that Phil did it
We believe that the work in \cite{hyatt2020parameterized} did not fully exercise the benefits of longer time horizons % I am not sure which benefits Phil didn't exercise here
